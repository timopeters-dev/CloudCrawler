...
with st.container():
    ...
    with col_a:
        base_url = st.text_input(
            "Basis-URL", "http://books.toscrape.com/catalogue/page-{}.html"
        )
    with col_b:
        ...
        if not queue_url:
            ...
            with st.spinner(f"Sende {msg_count} Aufgaben an SQS..."):
                for i in range(msg_count):
                    target_url = (
                        base_url.format(i + 1) if "{}" in base_url else base_url
                    )
...
def show_data():
    ...
    results_cursor = (
        db["results"].find().sort("_id", -1).limit(200)
    )  # Limit leicht erhöht für bessere Charts
    ...
    tab1, tab2, tab3 = st.tabs(
        ["📊 Daten-Tabelle", "📈 Analysen & Charts", "⚠️ Fehler-Log"]
    )
    ...
    with tab2:
        ...
        if df.empty:
            ...
            if "scraped_at" in df.columns:
                ...
                st.caption(
                    "Diese Kurve zeigt die parallele Arbeitskraft deiner Worker-Container."
                )
                st.markdown("---")
    ...
    with tab3:
        ...
        if errors_list:
            for e in errors_list:
                with st.expander(
                    f"Fehler: {e.get('error', 'Unbekannt')} (Klick für Details)"
                ):
                    st.json(e)
        else:
